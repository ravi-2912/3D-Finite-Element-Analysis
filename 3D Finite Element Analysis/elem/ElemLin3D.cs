...
namespace FEA3D.elem
{
    ...
    public class ElementLin3D : Element
    {
        ...
        private static double[][] dnxy = new double[8][]{ new double[3], new double[3], new double[3], new double[3], 
        ...
        public override void stiffnessMatrix()
        {
            ...
            for (int i = 0; i < 24; i++)
                for (int j = i; j < 24; j++) kmat[i][j] = 0.0;
            // Material mat
        ...
        // fn [8][6] - stresses at nodes (out)
        public override void extrapolateToNodes(double[][] fip, double[][] fn)
        {
            // Vertices
            int[] vn = new int[8] { 0, 1, 2, 3, 4, 5, 6, 7 };
